...
defmodule AOC2023.MirageMaintenance do
  def solve do
  ...
  Examples:
      iex> AOC2023.MirageMaintenance.get_extrapolated_value_from_history_entry([1, 2, 3, 4], [])
      # level 0:  1 2 3 4
      # level -1:  1 1 1
      # level -2:   0 0
      # so, extrapolated is:
      ...
      iex> AOC2023.MirageMaintenance.get_extrapolated_value_from_history_entry([1, 3, 6, 10, 15, 21], [])
  ...
  def get_extrapolated_value_from_history_entry(current_history_seq, history_seqs) do
    ...
    if is_current_seq_all_zeros? do
      ...
      |> Enum.reduce(0, fn seq, acc ->
        last_value_from_seq = Enum.at(seq, -1)
        acc + last_value_from_seq
      end)
    else
      next_history_seq =
        current_history_seq
